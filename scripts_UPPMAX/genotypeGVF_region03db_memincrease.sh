...
#SBATCH -J GenotypeGVCF_reg03  # Job name
#SBATCH -e GenotypeGVCF_reg03_%j.err  # Standard error file, %j will be replaced with job ID
#SBATCH -o GenotypeGVCF_reg03_%j.out  # Standard output file, %j will be replaced with job ID

module load bioinfo-tools GATK/4.3.0.0

# GATK command for variant calling
gatk --java-options "-Xmx72g" GenotypeGVCFs \
     --variant gendb:///crex/proj/uppstore2019097/popgen/gvcf/DBI_by_regionsD/region3db \
     -all-sites \
     --output ../Pijimae.region03.allsitesNew.vcf.gz \
     --reference /crex/proj/uppstore2019097/popgen/PhylloscopusRefGenome_copyD/GCA_025389015.1_LUNI_Pthr_2_genomic.fna.gz \
     -L VHQE01000001.1:1-55738393
